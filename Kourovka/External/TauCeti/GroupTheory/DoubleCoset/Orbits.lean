...
public import Kourovka.External.TauCeti.GroupTheory.GroupAction.Burnside
public import Kourovka.External.TauCeti.GroupTheory.GroupAction.Stabilizer
  ...
  | H a =>
    ...
    | H b =>
      ...
      · rintro ⟨x, hx, y, hy, hr⟩
        ...
        have hyp : (⟨y, hyH⟩ : ℋ) ∈ stabilizer ℋ p := by
          change y • p = p
          exact hy
        refine ⟨⟨(⟨y, hyH⟩ : ℋ)⁻¹, inv_mem hyp⟩, ?_⟩
        ...
        change (⟨y, hyH⟩ : ℋ)⁻¹ • (b : ℋ ⧸ 𝒢.subgroupOf ℋ) = _
        rw [Quotient.smul_coe, smul_eq_mul, QuotientGroup.eq,
      ...
      · rintro ⟨s, hs⟩
        change (s : ℋ) • (b : ℋ ⧸ 𝒢.subgroupOf ℋ) = _ at hs
        rw [Quotient.smul_coe, smul_eq_mul, QuotientGroup.eq,
