...
import Kourovka2135.OddPSLTwoNonsplitDihedral
import Kourovka2135.CentralPGroupTransfer

/-! The central binary kernel bound for PSL2 over a field of cardinality
congruent to three modulo four. The proof uses actual transfer to the preimage
of the nonsplit dihedral subgroup. No multiplier or cohomology premise is used.
-/

set_option autoImplicit false
noncomputable section

namespace Kourovka2135.OddPSLTwoNonsplitCentralKernel
...
most two when the nonsplit dihedral subgroup has odd index. -/
theorem card_ker_le_two (hmod : Nat.card F % 4 = 3)
    (π : G →* Q F) (hπ : Function.Surjective π)
    (hbinary : IsPGroup 2 π.ker) (hcentral : π.ker ≤ Subgroup.center G) :
    Nat.card π.ker ≤ 2 := by
  obtain ⟨D, hDindex, da, db, hgen, hconj⟩ :=
    OddPSLTwoNonsplitDihedral.exists_odd_index_subgroup F hmod
  let P := D.comap π
  let φ : P →* D :=
...
theorem card_normal_le_two (hmod : Nat.card F % 4 = 3)
...
end Kourovka2135.OddPSLTwoNonsplitCentralKernel
