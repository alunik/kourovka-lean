...
private theorem checked : ∀ k : Fin 64,
      groupElement ⟨192 + k.val, by omega⟩ • regularVector = regularVector →
        192 + k.val = 0 := by
    decide +kernel

theorem regularChunk003 (i : Fin 1152)
    (_hlo : 192 ≤ i.val) (hhi : i.val < 256) :
    groupElement i • regularVector = regularVector → groupElement i = 1 := by
  let k : Fin 64 := ⟨i.val - 192, by omega⟩
  have heq : (⟨192 + k.val, by omega⟩ : Fin 1152) = i := by
    apply Fin.ext
    dsimp [k]
    omega
  intro hg
