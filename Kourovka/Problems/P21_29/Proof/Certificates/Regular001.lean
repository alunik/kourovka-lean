...
private theorem checked : ∀ k : Fin 64,
      groupElement ⟨64 + k.val, by omega⟩ • regularVector = regularVector →
        64 + k.val = 0 := by
    decide +kernel

theorem regularChunk001 (i : Fin 1152)
    (_hlo : 64 ≤ i.val) (hhi : i.val < 128) :
    groupElement i • regularVector = regularVector → groupElement i = 1 := by
  let k : Fin 64 := ⟨i.val - 64, by omega⟩
  have heq : (⟨64 + k.val, by omega⟩ : Fin 1152) = i := by
    apply Fin.ext
    dsimp [k]
    omega
  intro hg
