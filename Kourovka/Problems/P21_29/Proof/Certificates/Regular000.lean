...
private theorem checked : ∀ k : Fin 64,
      groupElement ⟨0 + k.val, by omega⟩ • regularVector = regularVector →
        0 + k.val = 0 := by
    decide +kernel

theorem regularChunk000 (i : Fin 1152)
    (_hlo : 0 ≤ i.val) (hhi : i.val < 64) :
    groupElement i • regularVector = regularVector → groupElement i = 1 := by
  let k : Fin 64 := ⟨i.val - 0, by omega⟩
  have heq : (⟨0 + k.val, by omega⟩ : Fin 1152) = i := by
    apply Fin.ext
    dsimp [k]
    omega
  intro hg
